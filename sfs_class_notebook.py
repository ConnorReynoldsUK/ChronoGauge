...
class SFS_hub(object):
    def __init__(self, i_gene, X_data, Y_data, X_test, Y_test, rhythmic_scores=None, error_threshold=60):
    ...
    def phase_selection(self, count_num, used_genes):
        ...
        if count_num == 0:
            np.random.seed()
            used_genes = self.rhythmic_scores.loc[self.genes_perm[0]]['phase']
            # make sure genes aren't repeated
            # used_genes = used_genes.loc[[i for i in used_genes.index ]]
            print(used_genes)
            for j in used_genes:
    ...
    def run_model(self, i_gene, X_data, Y_data, X_test, Y_test, type=None):
        ...
        for n_fold, (train_idx, valid_idx) in enumerate(self.folds.split(X_data, Y_data)):
            ...
            all_test_preds.append(normalize(model.predict(X_t)))
            tf.keras.backend.clear_session()
            tf.compat.v1.reset_default_graph()
            model = None
            del model
        ...
        if type == 'foward':
            ...
            self.results_iteration['test_error'].append(
                    self.cyclical_loss(Y_test.astype('float64'), np.mean(all_test_preds, axis=0).astype('float64')))
        # reverse stage
        if type == 'reverse':
            self.results_remove['train_error'].append(
                self.cyclical_loss(Y_data.astype('float64'), all_preds.astype('float64')))
            self.results_remove['test_error'].append(
                self.cyclical_loss(Y_test.astype('float64'),
    ...
    def add_genes(self, custom_genes):
        ...
        self.run_model(self.i_gene, self.X_data, self.Y_data, self.X_test, self.Y_test, 'foward')
    ...
    def manual_run(self, used_genes=None):
        ...
        for j in tqdm.tqdm(range(self.i_genes.shape[0])):
            ...
            self.run_model(i_gene, self.X_data, self.Y_data, self.X_test, self.Y_test, 'foward')
            print(phase)
            print('Phase run: {}'.format(phase))
            print('Iteration genes: ' , self.results_iteration['genes'][-1])
            print('Iteration error {}'.format(self.results_iteration['train_error'][-1]))
    ...
    def manual_reverse(self):
        ...
        while remove == True and len(self.genes_perm) > 3:
            ...
            for m in range(0, len(self.genes_perm) - 1):
                ...
                self.run_model(gene_remove, self.X_data, self.Y_data, self.X_test, self.Y_test, type='reverse')
    ...
    def status_update(self):
        ...
        for j in used_genes.values:
            counts[str(j)] += 1
        print(pd.DataFrame.from_dict(counts, orient='index'))
